...
- **Redundant R&D, brittle UX.** Per-app voice stacks are expensive to build, hard to maintain, and never as capable as a user-chosen agent with proper tools.

If you’re building a voice UI into a single product, ask: *Am I a data source, or am I the primary actor the user truly wants to talk to?* Most apps are sources. That’s not an insult; it’s a design truth.
...
**Why this misses:** contracts live in Drive, mail, Slack, Notion, and history. The useful task (“compare to last year; draft legal’s note; propose redlines”) spans storage + email + calendar. The viewer shouldn’t own that voice.
...
### D) Zoom AI Companion: another voice in the meeting window
...
**Why this teaches the lesson:** maintaining voice per surface is brittle. The durable value concentrates at the **agent layer** that spans repos, issues, docs, and chat—not one editor.
...
Vendors should **ship tools and authenticated access**, not their own voice overlord:
...
Call it MCP-ish if you like; call it “tools over voices.” The point is the same: **apps provide data and actions; the agent provides the conversation and the plan**.
...
1. *Are you a destination or a datasource?*  
   If most valuable user intents require other systems, you’re a **source**—optimize for tools and access, not owning the voice.

2. *Does your voice UI meaningfully outperform the user’s agent with your tools attached?*  
...
We can still ship “starter” voices inside products for *debugging and testing your tools*. Just don’t confuse that with the endgame. The endgame is simple:
